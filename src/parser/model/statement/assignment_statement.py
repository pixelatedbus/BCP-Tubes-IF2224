...
def parse_assignment_statement(parser):
    ...
    # Handle array indexing and/or record field access
    while True:
        ...
        elif token and token[0] == "DOT":
            dot_node = ParseNode(f"DOT({token[1]})")
            assignment_node.add_child(dot_node)
            parser.advance()
            
            field = parser.check_token("IDENTIFIER")
            assignment_node.add_child(ParseNode(f"IDENTIFIER({field[1]})"))
        else:
            break
